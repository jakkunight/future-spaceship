...
export default function Display({ children, ...props }: DisplayParams) {
  return <div
    ...
    //className="w-full min-h-screen h-full text-white text-4xl font-bigblueterm shadow-[inset_0px_0px_2rem] shadow-cyan-500 bg-[100% 80%] bg-repeat-y"
    {...props} >
    <div
      className="scanlines-gray-500 w-full min-h-screen h-full flex flex-col items-center justify-center" {...props}>
      {children}
    </div>
  </div>;
}
